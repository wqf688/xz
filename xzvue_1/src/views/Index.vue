...
export default {
  data(){
    return{
      ...
      p2:{price:0,href:""},
      p3:{price:0,href:""},
      //因为others是个空数组，v-for根本不会执行
      others:[]

    }
  },
  beforeCreate(){
    console.log(`创建当前组件页面对象前，自动执行....`)
  },
  created(){
    console.log(`当前页面组件页面创建完成，但还未挂载虚拟dom树页面，自动执行...`)
    // this.axios.get("/index")
    // .then(res=>{//必须用箭头函数，保证this指当前vue对象本身
    //   res.data;//
    //   console.log(res.data)
    //   this.p1=res.data[0];
    //   this.p2=res.data[1];
    //   this.p3=res.data[2];
    //   this.others=res.data.slice(-3);//取3个商品的对象，组成子对象
    // })
      getIndex().then(
        result=>{
          console.log(result);
  ...
  beforeMount(){
    console.log(`挂载当前页面虚拟dom树之前，自动执行...`)
  },
   mounted(){
    console.log(`挂载当前页面虚拟dom树完成之后，自动执行...`)
  },
  components:{
    Carousel
    }
